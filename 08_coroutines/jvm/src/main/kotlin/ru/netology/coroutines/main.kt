        ...
        launch {
            try {
                val posts = getPosts(client)
                    .map { post ->
                        async {
                            PostsWithAuthors(post, getAuthors(client, post.authorId))
                            PostWithComments(post, getComments(client, post.id)).comments.map { comment ->
                                async {
                                    CommentsWithAuthors(comment, getAuthors(client, comment.authorId))
                                }
                            }
                        }
                    }.awaitAll()
